...
export function SpotSelector({
    ...
    return (
        <div ref={ref} className={cn('relative', className)}>
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    <div className="text-xs font-medium text-foreground truncate">
                        {currentSpot?.name || 'Выберите спот'}
                    </div>
                    {currentSpot && (
                        <div className="text-[10px] text-muted-foreground">
                            {currentSpot.format} • {currentSpot.positions.join(' vs ')}
                        </div>
                    )}
                </div>
                {open ? (
            ...
            {open && (
                <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-card border border-border rounded-xl shadow-xl max-h-[400px] flex flex-col overflow-hidden min-w-[240px]">
                            ...
                            return (
                                <div key={fmt} className="mb-2">
                                    <div className={cn('text-[10px] font-bold uppercase tracking-wider px-2 py-1', FORMAT_COLORS[fmt] || 'text-muted-foreground')}>
                                        {fmt === '3bet' ? '3-Bet' : fmt === '4bet' ? '4-Bet' : fmt === 'squeeze' ? 'Squeeze' : 'SRP'}
